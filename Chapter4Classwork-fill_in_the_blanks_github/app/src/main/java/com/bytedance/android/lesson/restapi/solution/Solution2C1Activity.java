    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRv.setAdapter(new Adapter() {
            ...
            public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
                ...
                Glide.with(iv.getContext()).load(((Cat) Solution2C1Activity.this.mCats.get(i)).getUrl()).into(iv);
...
//                Glide.with(iv.getContext()).load(url).into(iv);
    ...
    public void requestData(View view) {
        ...
        ((ICatService) RetrofitManager.get(ICatService.HOST).create(ICatService.class)).randomCat(5).enqueue(new Callback<List<Cat>>() {
            public void onResponse(Call<List<Cat>> call, Response<List<Cat>> response) {
                Solution2C1Activity.this.restoreBtn();
                Solution2C1Activity.this.loadPics((List) response.body());
            }
